...
**필요한 두 가지.** 스킬들의 `SKILL.md` 가 호출하는 CLI 커맨드(`wiki init`, `wiki log inspect`, `wiki sync`, ...)는 이 저장소의 Python 패키지가 제공하는 단일 `wiki` 명령의 서브커맨드입니다. 따라서 (1) Python 패키지 설치, (2) Claude Code 에 plugin 등록 — 두 단계가 필요합니다.

```bash
pip install researchwiki         # (1) `wiki` CLI 설치 — PyPI 게시 후 사용 가능
```

> PyPI 게시 전(현재) 까지는 Git 직접 설치:
> ```bash
> pip install git+https://github.com/apple4ree/researchwiki.git
> ```

Claude Code 에 plugin 등록 — 두 갈래 중 하나:

```bash
# (a) self-hosted marketplace 경유 — 권장
...
```bash
# (b) 로컬 클론을 직접 plugin-dir 로 지정 — 개발자용
git clone https://github.com/apple4ree/researchwiki.git
cd researchwiki && pip install -e . && claude --plugin-dir .
```

이후 Claude Code 안에서 `/researchwiki:wiki-init`, `/researchwiki:wiki-log` 등 namespace 된 슬래시 커맨드로 호출합니다.

> **CLI 개요.** `wiki <subcommand> [args]` 형태. 8개 서브커맨드: `init`, `log`, `sync`, `lint`, `deepscan`, `query`, `recall`, `fix-stale`. `wiki --help` 로 전체 목록, `wiki <sub> --help` 로 옵션 확인. Class A 7개는 순수 Python, `wiki log` 만 Python + LLM 하이브리드입니다 — 자세한 구분은 `ARCHITECTURE.md §3.5` 참조.

> 향후 [Anthropic 공식 marketplace](https://platform.claude.com/plugins/submit) 에 제출되어 승인되면 marketplace add 단계가 생략되고 `/plugin install researchwiki@<official-marketplace>` 한 줄만 남습니다. 현재는 self-hosted marketplace 와 로컬 plugin-dir 두 경로만 지원.
