...
**필요한 두 가지.** 스킬의 `SKILL.md` 들이 호출하는 CLI 커맨드(`wiki-init`, `wiki-log`, `wiki-sync`, ...)는 이 저장소의 Python 패키지로 제공되므로 (1) Python 패키지 설치, (2) Claude Code에 plugin 디렉토리 알려주기 — 두 단계가 필요합니다.

```bash
git clone https://github.com/apple4ree/researchwiki.git
cd researchwiki
pip install -e .            # (1) CLI 명령 설치
claude --plugin-dir .       # (2) Claude Code 가 이 저장소를 plugin으로 로드
```

이후 Claude Code 안에서 `/researchwiki:wiki-init`, `/researchwiki:wiki-log` 등 namespace 된 슬래시 커맨드로 호출합니다. (다른 디렉토리에서 띄우려면 `claude --plugin-dir /path/to/researchwiki` 처럼 절대 경로 지정.)
