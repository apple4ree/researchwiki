...
이후 `/researchwiki:wiki-init` 같은 슬래시 커맨드를 호출하면 plugin 의 `bin/wiki` 부트스트래퍼가 처음 한 번 동작 시 `researchwiki` Python 패키지를 자동으로 `pip install` 합니다 (PyPI → 실패 시 GitHub 소스 fallback). 두 번째 호출부터는 import 체크만 거쳐 곧장 디스패치 (~300ms 오버헤드).

> **사전 조건은 Python 3.10+ 만.** `python3` 이 PATH 에 있고 버전이 3.10 이상이면 충분합니다. pip 설치 위치는 자동 판단 — venv 안이면 venv 에, 시스템 Python 이면 `--user` 사이트에. PEP 668 (Debian 12+, Homebrew Python) 환경에서 자동 install 이 막히면 명확한 안내와 함께 `pipx` / venv 사용을 권장.
...
> **부트스트랩 비활성화.** CI / 컨테이너 환경에서 자동 install 을 끄려면 `WIKI_NO_BOOTSTRAP=1` env 설정. 이 경우 `researchwiki` 가 미리 설치되어 있어야 합니다.

> **향후 PyPI 게시 시.** [Anthropic 공식 marketplace](https://platform.claude.com/plugins/submit) 에 제출돼 승인되면 marketplace add 단계조차 생략되고 `/plugin install researchwiki@<official>` 한 줄만 남습니다.
...
소스는 `src/researchwiki/` 아래에 있고, 단일 `wiki` CLI 가 `pip install` 시 PATH 에 등록됩니다. 8개 스킬은 그 CLI 의 서브커맨드로 노출됩니다 (`wiki init`, `wiki log`, ...). 168개 테스트 모두 통과 (159 unit + 9 integration).
