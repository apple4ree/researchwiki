...
**원하는 건 두 줄 뿐입니다.** Claude Code 안에서:
...
이후 `/researchwiki:wiki-init` 같은 슬래시 커맨드를 호출하면 plugin 의 `bin/wiki` 부트스트래퍼가 처음 한 번 동작 시 `researchwiki` Python 패키지를 GitHub 소스에서 자동으로 `pip install` 합니다. 두 번째 호출부터는 import 체크만 거쳐 곧장 디스패치 (~300ms 오버헤드).

> **사전 조건은 Python 3.10+ 만.** `python3` 이 PATH 에 있고 버전이 3.10 이상이면 충분합니다. pip 설치 위치는 자동 판단 — venv 안이면 venv 에, 시스템 Python 이면 `--user` 사이트에. PEP 668 (Debian 12+, Homebrew Python) 환경에서 자동 install 이 막히면 명확한 안내와 함께 `pipx` / 전용 venv 사용을 권장.

> **PyPI 미게시.** 이 프로젝트는 의도적으로 PyPI 에 게시하지 않고 GitHub 소스만으로 배포합니다. 부트스트래퍼가 항상 `pip install git+https://github.com/apple4ree/researchwiki.git` 로 설치하므로 라이선스 결정 / 버저닝 / 게시 단계가 필요 없습니다.

### 개발자용 — 로컬 클론으로 plugin 로드

```bash
git clone https://github.com/apple4ree/researchwiki.git
cd researchwiki && pip install -e . && claude --plugin-dir .
```

> **CLI 개요.** `wiki <subcommand> [args]` 형태. 8개 서브커맨드: `init`, `log`, `sync`, `lint`, `deepscan`, `query`, `recall`, `fix-stale`. `wiki --help` 로 전체 목록, `wiki <sub> --help` 로 옵션. Class A 7개는 순수 Python, `wiki log` 만 Python + LLM 하이브리드 — 자세한 구분은 `ARCHITECTURE.md §3.5`.

> **환경 변수.** `WIKI_NO_BOOTSTRAP=1` — 자동 install 비활성화 (CI / 컨테이너에서 미리 설치된 환경용). `WIKI_UPGRADE=1` — 다음 호출 시 최신 main 으로 강제 재설치.
...
소스는 `src/researchwiki/` 아래에 있고, 단일 `wiki` CLI 가 `pip install` 시 PATH 에 등록됩니다. 8개 스킬은 그 CLI 의 서브커맨드로 노출됩니다 (`wiki init`, `wiki log`, ...). 196개 테스트 모두 통과 (187 unit + 9 integration).
...
| `wiki-log` | **B** | 수시 | 대화에서 후보 추출 → 배치 승인 (a/r/f/d 4-tier · 마찰 다이얼) | `wiki/<kind>/<slug>.md`, `log.md` append, `index.md` 갱신, 양방향 back-ref |
...
- **`wiki-log` v0.2 (Python + LLM 하이브리드, conversation-as-source)** — *유일한* LLM 필수 스킬입니다. v0.2 부터 디폴트 흐름이 *인터뷰 → 작성* 에서 *대화 → 추출 → 검증 → 배치 승인* 으로 바뀌었습니다. Mechanical core (`src/researchwiki/log.py` + 6개 CLI 서브커맨드 `inspect`, `lookup-symbols`, `find-pages`, `find-amend-target`, `validate-candidate`, `run`) 와 P8 1차 필터 (`src/researchwiki/p8.py` — KO/EN 헤지·인과·의도·implication regex 26개) 가 결정론적 부분 담당. LLM 은 (1) 최근 대화에서 후보 추출, (2) `wiki log validate-candidate` 로 status (`ok` / `needs-review` / `fatal`) + 이슈 (missing-required, p8-marker, ref-unresolved, stub-candidate, collision) 받음, (3) 4-tier 배치 UI 로 연구자에게 제시 — `[a]` 자동저장 (P8 마커는 `[speculation]` 인라인 태깅) / `[r]` 플래그된 항목만 review (default, 1~3 질문) / `[f]` legacy full interview (12~15 질문) / `[d]` drop. 마찰 다이얼: `--strict` (모두 review), default, `--quick` (모두 자동저장 + lint 사후 검증). `[a]` tier 로 만들어진 entry 는 `auto_extracted: true` frontmatter 마커. `authored_by: llm` 은 validator 가 거부 — 모든 entry 는 사람의 의도를 요구합니다. 가이드 reference 9개 문서 (`p8-detection.md`, `conversational-style.md`, `auto-link-extraction.md`, `refusal-patterns.md`, `templates-deep-dive.md` 등). CLI: `wiki log {inspect | lookup-symbols | find-pages | find-amend-target | validate-candidate | run} ...`.
